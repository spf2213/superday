...
function buildSystemPrompt(mode, firm, category, difficulty, levels) {
  if (mode === 'mock_interview') {
    ...
    return [
      ...
      '',
      'PART 2 — Feedback in EXACTLY this format, no extra prose, no markdown headers, no surrounding commentary. Each line begins with the literal label shown:',
      'GAP: <one short sentence, ≤20 words, naming the single biggest thing the answer missed or got wrong>',
      'FIX:',
      '- <one specific, concrete thing to add or correct, ≤16 words>',
      '- <one specific, concrete thing to add or correct, ≤16 words>',
      '- <optional third item, ≤16 words; omit the bullet entirely if not needed>',
      'STRONGER: <one sentence describing what a 9/10 answer at this topic and band would include>',
      '',
      'Be concrete in every line: name concepts, frameworks, specific numbers, or precise phrasings the candidate should have used. Confident but kind: direct, not snarky, not coddling. Never use markdown bold/italic — only the labels and the dash bullets shown above.',
      '',
